...
var inspectCmd = &cobra.Command{
	Use:   "inspect",
	Short: "Inspect allows to inspect SunSpec devices",
	Long: `Inspec will iterate across the SunSpec model definition for the specified device
and print details about defined models and data points (EXPERIMENTAL)`,
	Run: inspect,
}
		...
		d.Do(func(m sunspec.Model) {
			...
			m.Do(func(b sunspec.Block) {
				...
				b.Do(func(p sunspec.Point) {
					t := p.Type()[0:3]
					v := p.Value()
					if p.NotImplemented() || (p.Type() == "sunssf" && int16(p.ScaleFactor()) == int16(math.MinInt16)) {
						v = "n/a"
